...
<template>
    ...
    <select class="border-cyan-300 focus:border-sky-300 focus:ring focus:ring-sky-200 focus:ring-opacity-50 rounded-md shadow-sm" @change="$emit('update:modelValue', $event.target.value)" ref="input">
        ...
        <option v-for="plano in planos" :value="`${plano.id}`" :tempoPHP="`${plano.tempoPHP}`" :qtdAtendimentos="`${plano.atendimentos}`" :frequencia="`${plano.frequencia}`" >{{plano.nome}} - {{plano.tempo}} - {{plano.frequencia}}</option>
    </select>
</template>
